...
def main():
    ...
    :return: None
    """
    title = 'rRT, without batches, fine-tuned features'
    print(title)
    ...
    # print options used
    print(Options)
    mb.train_reg_head(feature_extractor_plus_head, shuffled_train_x, shuffled_train_y, shuffled_val_x, shuffled_val_y,
    ...
    plot_tsne_extended(feature_extractor_plus_head, combined_train_x, combined_train_y, title,
                                'rrt_stage1_training_',
                                save_tag=timestamp)

    plot_tsne_extended(feature_extractor_plus_head, shuffled_test_x, shuffled_test_y, title,
                                'rrt_stage1_testing_',
                                save_tag=timestamp)

    # add the regression head with dense weighting
    regressor = mb.add_reg_proj_head(feature_extractor_plus_head, freeze_features=False)
    ...
    plot_tsne_extended(regressor, combined_train_x, combined_train_y, title, 'rrt_stage2_training_',
                                save_tag=timestamp)

    plot_tsne_extended(regressor, shuffled_test_x, shuffled_test_y, title, 'rrt_stage2_testing_',
                                save_tag=timestamp)
