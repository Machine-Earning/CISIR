...
def main():
    ...
    alphas = [(0.9, 0.9, 0.0, 0.0)]
    alpha_amse = alphas[0][0]
    alpha_apcc = alphas[0][2]
    lambda_factor = 0.7 # LAMBDA_FACTOR  # lambda for the loss
    ...
    results['name'] = f'onp_mlp_amse{alpha_amse:.2f}_apcc{alpha_apcc:.2f}_lambda{lambda_factor:.2f}_mdi'
    ...
    for seed_idx, seed in enumerate(TRIAL_SEEDS):
        for alpha_mse, alphaV_mse, alpha_pcc, alphaV_pcc in alphas:
            for rho in RHO:  # SAM_RHOS:
                # PARAMS
                # Construct the title
                title = f'mlp_amse{alpha_mse:.2f}_apcc{alpha_pcc:.2f}_lambda{lambda_factor:.2f}_mdi'
