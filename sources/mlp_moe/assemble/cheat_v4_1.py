    ...
    for seed in SEEDS:
        for alpha_mse, alphaV_mse, alpha_pcc, alphaV_pcc in REWEIGHTS_MOE:
            for rho in RHO_MOE:  # SAM
                ...
                # Construct the title
                title = f'mlp2_amse{alpha_mse:.2f}_moe_cheat_v4_pccce'
                ...
                init_model_sep.compile(
                    ...
                    loss={
                        ...
                        'combiner': lambda y_true, y_pred: pcc_ce(
                            y_true, y_pred,
                            phase_manager=pm,
                            lambda_ce=lambda_ce,
                            train_ce_weight_dict=mse_train_weights_dict,
                            val_ce_weight_dict=mse_test_weights_dict,
                            train_pcc_weight_dict=pcc_train_weights_dict,
                            val_pcc_weight_dict=pcc_test_weights_dict
                        )
                    },
                ...
                train_ds = train_dataset.map(
                    lambda x, y_cls, delta: (
                        x, {
                            'forecast_head': delta,
                            'combiner': tf.concat([y_cls, tf.expand_dims(delta, axis=-1)], axis=1)
                        }
                    )
                )
                test_ds = test_ds.map(
