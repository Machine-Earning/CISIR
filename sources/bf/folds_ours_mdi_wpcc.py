...
def main():
    ...
    results['name'] = f'blogf_mlp_amse{alpha_amse:.3f}_apcc{alpha_apcc:.2f}_lambda{lambda_factor:.2f}_mdi_wpcc'
    ...
    for seed_idx, seed in enumerate(TRIAL_SEEDS):
        for alpha_mse, alphaV_mse, alpha_pcc, alphaV_pcc in alphas:
            for rho in RHO:  # SAM_RHOS:
                # PARAMS
                # Construct the title
                title = f'mlp_amse{alpha_mse:.3f}_apcc{alpha_pcc:.2f}_lambda{lambda_factor:.2f}_mdi_wpcc'
