...
def weighted_mse_loss(y_true: tf.Tensor, y_pred: tf.Tensor, gamma: float, eta: float = 1,
                      epsilon: float = 1e-20) -> tf.Tensor:
    """
    Compute the Mean Squared Error weighted by a factor of (target_value / divisor)^gamma, with an additional penalty
    for positive errors.
    ...
    - eta (float): Penalty factor for positive errors (underestimations).
    - epsilon (float, optional): A small value to avoid division by zero. Defaults to 1e-6.

    Returns:
    - tf.Tensor: The computed weighted MSE loss with additional penalty for positive errors.
    """
    divisor = 4
    ...
    # Calculate additional penalty for positive errors
    positive_error_penalty = tf.where(y_true > y_pred, eta * mse, 0.0)

    # Apply the weights and additional penalty
    weighted_mse = weights * (mse + positive_error_penalty)
...
def main():
    ...
    """
    gammas = [0, 4]
    etas = [0.1, 0.5, 0.7, 1.2]  # Define a range of eta values
    for gamma in gammas:
        for eta in etas:
            for inputs_to_use in [['e0.5', 'e1.8', 'p']]:
                for add_slope in [False]:
                    ...
                    experiment_name = f'{title}_gamma{gamma}_eta{eta}_{current_time}'

                    # Initialize wandb
                    wandb.init(project="mlp-ts-gamma-eta", name=experiment_name, config={
                    ...
                    X_subtrain, y_subtrain = build_dataset(root_dir + '/subtraining', inputs_to_use=inputs_to_use,
                                                           add_slope=add_slope)
                    X_test, y_test = build_dataset(root_dir + '/testing', inputs_to_use=inputs_to_use,
                                                   add_slope=add_slope)
                    X_val, y_val = build_dataset(root_dir + '/validation', inputs_to_use=inputs_to_use,
                                                 add_slope=add_slope)
                    ...
                    mlp_model_sep.compile(optimizer=AdamW(learning_rate=learning_rate,
                                          ...
                                          loss={'forecast_head':
                                                    lambda y_true, y_pred: weighted_mse_loss(y_true, y_pred, gamma, eta)
                                                })
                    ...
                    final_mlp_model_sep.compile(optimizer=AdamW(learning_rate=learning_rate,
                                                                ...
                                                                beta_1=momentum_beta1),
                                                loss={'forecast_head':
                                                          lambda y_true, y_pred: weighted_mse_loss(y_true, y_pred,
                                                                                                   gamma, eta)
                                                      })
