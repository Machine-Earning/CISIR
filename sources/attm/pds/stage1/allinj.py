                ...
                for alpha, alphaV in [(2, 1)]:
                    for rho in [1e-1]:
                        for add_slope in ADD_SLOPE:
                            ...
                            # Construct the title
                            title = f'ATTM_PDSallinj_bs{batch_size}_alpha{alpha:.2f}_rho{rho:.2f}'
                            ...
                            for fold_idx, (X_subtrain, y_subtrain_norm, X_val, y_val_norm) in enumerate(
                                ...
                                # injection batch dataset
                                subtrain_ds, subtrain_steps = mb.inject_batch_dataset(
                                    X_subtrain, y_subtrain_norm,
                                    batch_size, n_inj,
                                    norm_lower_t, norm_upper_t,
                                    phase_name="subtrain"
                                )
                            ...
                            # injection batch dataset
                            train_ds, train_steps = mb.inject_batch_dataset(
