        ...
        for add_slope in [False, True]:
            for cme_speed_threshold in [0]:
                for alpha in [0]:
                    ...
                    # Construct the title
                    title = f'MLP_{inputs_str}_PDS_Stage1_alpha{alpha:.2f}'
                    ...
                    residual = True
                    skipped_layers = 2
                    N = 1000  # number of samples to keep outside the threshold
                    lower_threshold = -0.5  # lower threshold for the delta_p
                    upper_threshold = 0.5  # upper threshold for the delta_p
                    # Initialize wandb
                    wandb.init(project="nasa-ts-delta-plot", name=experiment_name, config={
