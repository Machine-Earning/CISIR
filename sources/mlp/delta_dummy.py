...
from modules.training.ts_modeling import (
    ...
    create_mlp,
    evaluate_model,
evaluate_model_dummy,
evaluate_model_cond_dummy,
    evaluate_model_cond,
    process_sep_events,
    get_loss,
    reshape_X)
    ...
    for inputs_to_use in [['e0.5', 'e1.8', 'p']]:
        for add_slope in [True, False]:
            for alpha in [0.1]:
                ...
                # Initialize wandb
                wandb.init(project="nasa-ts-delta-dummy", name=experiment_name, config={
                ...
                error_mae = evaluate_model_dummy(y_test)
                print(f'mae error: {error_mae}')
                # Log the MAE error to wandb
                wandb.log({"mae_error": error_mae})

                # evaluate the model on training cme_files
                error_mae_train = evaluate_model_dummy(y_train)
                ...
                # evaluate the model on test cme_files
                above_threshold = 0.1
                error_mae_cond = evaluate_model_cond_dummy(y_test, above_threshold=above_threshold)
                ...
                # evaluate the model on training cme_files
                error_mae_cond_train = evaluate_model_cond_dummy(y_train, above_threshold=above_threshold)
