        ...
        for alpha, alphaV in REWEIGHTS_PRE:
            for rho in RHO_PRE:
                ...
                error_pcc_cond = evaluate_pcc_repr(
                    final_model_sep, X_test, y_test, i_above_threshold=upper_threshold)
                print(f'pcc error delta i>= {upper_threshold} test: {error_pcc_cond}')
                wandb.log({"jpcc+": error_pcc_cond})

                # evaluate pcc+ on the training set
                error_pcc_cond_train = evaluate_pcc_repr(
                    final_model_sep, X_train, y_train, i_above_threshold=upper_threshold)
                print(f'pcc error delta i>= {upper_threshold} train: {error_pcc_cond_train}')
