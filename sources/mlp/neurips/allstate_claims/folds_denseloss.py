...
from modules.evaluate.utils import plot_asc_corr, plot_tsne_asc
from modules.reweighting.ImportanceWeighting import DenseLossImportance
from modules.shared.asc_globals import *
...
def main():
    ...
    results['name'] = f'asc_mlp_amse{alpha_amse:.2f}_apcc{alpha_apcc:.2f}_denseloss'
    ...
    for seed_idx, seed in enumerate(TRIAL_SEEDS):
        for alpha_mse, alphaV_mse, alpha_pcc, alphaV_pcc in alphas:
            for rho in RHO:
                ...
                # Initialize wandb
                wandb.init(project="NeurIPS-2025-Paper-ASC", name=experiment_name, config={
                    "patience": patience,
                ...
                root_dir = DS_PATH
                # build the dataset
                X_train, y_train = build_asc_ds(
                    root_dir + '/allstate_claims_training.csv',
                    shuffle_data=True,
                    random_state=seed
                )
                ...
                X_test, y_test = build_asc_ds(
                    root_dir + '/allstate_claims_testing.csv',
                    shuffle_data=False,
                    random_state=seed
                )
                ...
                stage1_file_path = plot_tsne_asc(
                    final_model_sep,
    ...
    csv_filename = f"asc_results_{title}.csv"
