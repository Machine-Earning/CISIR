...
# Import our new balanced loss
from modules.training.bmse import create_gai_loss


def main():
    """
    Testing Balanced MSE with Gaussian Mixture Model
    """
    ...
    for seed in TRIAL_SEEDS:
        for rho in RHO:  # SAM_RHOS:
            ...
            # Parameters for GAILossMD
            n_components = 64  # Number of GMM components
            init_noise_sigma = 1.0  # Initial noise sigma
            ...
            wandb.init(project="NeurIPS-2025-Paper", name=experiment_name, config={
                ...
                "hiddens": hiddens_str,
                "loss": 'balanced_mse',  # Updated loss name
                "seed": seed,
                "embed_dim": embed_dim,
                "dropout": dropout,
            ...
            # Get the target variable for GMM fitting
            delta_train = y_train[:, 0]
            print(f'delta_train.shape: {delta_train.shape}')
            
            # Create the balanced MSE loss using the training data
            train_gai_loss = create_gai_loss(
                delta_train,
            ...
            for fold_idx, (X_subtrain, y_subtrain, X_val, y_val) in enumerate(
                ...
                # Get the target variable for this fold
                delta_subtrain = y_subtrain[:, 0]
                print(f'delta_subtrain.shape: {delta_subtrain.shape}')
                
                # Create the balanced MSE loss for this fold
                subtrain_gai_loss = create_gai_loss(
                    delta_subtrain,
                    n_components=n_components, 
                    init_noise_sigma=init_noise_sigma
                )
                ...
                model_sep.compile(
                    ...
                    loss={
                        'forecast_head': subtrain_gai_loss  # Use our new loss
                    }
                )
            ...
            # Compile the final model with balanced MSE
            final_model_sep.compile(
                optimizer=Adam(
                    learning_rate=learning_rate,
                ),
                loss={
                    'forecast_head': train_gai_loss  # Use our new loss with full training data
                }
            )
