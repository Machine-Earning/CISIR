...
def main():
    ...
    results['name'] = f'sep_cme_mlp_amse{alpha_amse:.2f}_apcc{alpha_apcc:.2f}_lambda{lambda_factor:.2f}_quc_s2_pds'
    ...
    for seed_idx, seed in enumerate(TRIAL_SEEDS):
        for alpha_mse, alphaV_mse, alpha_pcc, alphaV_pcc in alphas:
            for rho in RHO:  # SAM_RHOS:
                # PARAMS
                # Construct the title
                title = f'mlp_amse{alpha_mse:.2f}_apcc{alpha_pcc:.2f}_lambda{lambda_factor:.2f}_quc_s2_pds'
                ...
                wandb.init(project="2025-Papers-SEPC", name=experiment_name, config={
                    ...
                    'sep_threshold': sep_threshold,
                    'n_filter': n_filter,
                    'weights_path': PDS_WEIGHTS_PATH[seed]
                })
                ...
                for fold_idx, (X_subtrain, y_subtrain, X_val, y_val) in enumerate(
                    ...
                    print(f'weights loading from: {PDS_WEIGHTS_PATH[seed]}')
                    model_sep_s1.load_weights(PDS_WEIGHTS_PATH[seed])
                    # print the save
                    print(f'weights loaded successfully from: {PDS_WEIGHTS_PATH[seed]}')
                ...
                # load the weights from the first stage
                print(f'weights loading from: {PDS_WEIGHTS_PATH[seed]}')
                final_model_sep_s1.load_weights(PDS_WEIGHTS_PATH[seed])
                # print the save
                print(f'weights loaded successfully from: {PDS_WEIGHTS_PATH[seed]}')
