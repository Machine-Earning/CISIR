...
def main():
    """
    Testing Vanilla MSE implementation that:
    1. Doesn't use stratified batching
    2. Uses standard MSE loss without any weighting
    """
    ...
    results['name'] = 'electron_delta_mlp_vanilla_mse'
    ...
    for seed_idx, seed in enumerate(TRIAL_SEEDS):
        for rho in RHO:  # SAM_RHOS:
            ...
            # getting the delta values for training set
            ...
            for fold_idx, (X_subtrain, y_subtrain, X_val, y_val) in enumerate(
                ...
                # Compile the model with the specified learning rate and MSE loss
                model_sep.compile(
                    optimizer=Adam(
                        learning_rate=learning_rate,
                    ),
            ...
            # Compile the final model with MSE loss
    ...
    csv_filename = f"electron_delta_results_{title}.csv"
