...
def main():
    ...
    for batch_size in [292, len_train]:
        title = f'PDS, Dense Joint Loss, AE, {"with" if batch_size == 292 else "without"} batches'
        print(title)
        with mlflow.start_run(run_name=f"PDS_DL_AE_{batch_size}"):
            # Automatic logging
            ...
            mb.train_pds_dl_heads(feature_extractor,
                                  ...
                                  patience=Options['patience'], save_tag=timestamp + "_features_dec")

            # Log model to DagsHub
            mlflow.tensorflow.log_model(model=feature_extractor, artifact_path="pds_dl_ae_model")
