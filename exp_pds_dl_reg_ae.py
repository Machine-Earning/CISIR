...
def main():
    ...
    for batch_size in [292, -1]:
        title = f'PDS, Dense Joint Loss, Regression, AE, {"with" if batch_size > 0 else "without"} batches'
        print(title)
        with mlflow.start_run(run_name=f"PDS_DL_REG_AE_{batch_size}"):
            ...
            if recovery:
                weight_path = "/home1/jmoukpe2016/keras-functional-api/best_model_weights_2023-10-30_22-39-20_features_reg_dec.h5"
            ...
            mlflow.tensorflow.log_model(model=feature_extractor, artifact_path="pds_dl_reg_ae_model")
