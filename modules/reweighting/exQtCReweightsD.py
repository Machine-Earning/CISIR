...
class exDenseReweightsD:
    """
    Class for generating synthetic regression datasets with power-based reweighting.
    """
    # class variables
    debug = False
    X_train = None
    ...
    def __init__(self, X, y,
        ...
        :param alpha: reweighing coefficient for power function
    ...
    def reweight(self, y: ndarray, alpha: float, epsilon: float = 1e-6) -> ndarray:
        """
        Calculate the reweighting factor for a label y using power-based reweighting.
        ...
        normalized_density = density / (self.max_pdf + epsilon)
        # Apply power reweighting: y = [1- x^(alpha/2)]^(2/alpha)
        return np.power(1 - np.power(normalized_density, alpha/2), 2/alpha)
