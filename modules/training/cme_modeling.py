...
class ModelBuilder:
    ...
    def pdc_loss_vec(
        ...
        z_diff = tf.norm(
            z_pred[:, tf.newaxis, :] - z_pred[tf.newaxis, :, :] + tf.keras.backend.epsilon(),
            ord=2,  # L2 norm
            axis=-1
        )
    ...
    def pdc_loss_linear_vec2(
        ...
        """
        Computes the PDC (Pairwise Distance Correlation) loss with diagonal terms excluded,
        using absolute differences for labels and manual L2 distance calculation for representations.
        ...
        # Compute L2 distances manually for z differences
        fup_z = z_pred[:-1] - z_pred[1:]
        # Absolute differences for labels
        fup_y = y_true[:-1] - y_true[1:]

        # Remaining differences from first element
        rem_z = z_pred[0:1] - z_pred[2:]
        rem_y = y_true[0:1] - y_true[2:]

        # Concatenate all differences
        z_diff = tf.sqrt(tf.reduce_sum(tf.square(tf.concat([fup_z, rem_z], axis=0)), axis=-1) + eps)  # Shape: [2N-3]
        y_diff = tf.abs(tf.concat([fup_y, rem_y], axis=0))  # Shape: [2N-3]
    ...
    def pds_loss_vec(
        ...
        z_pred_diff = z_pred[:, tf.newaxis, :] - z_pred[tf.newaxis, :, :] + tf.keras.backend.epsilon()
