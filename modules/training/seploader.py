...
class SEPLoader:
    ...
    def load(self, file_path: str, num_folds: Optional[int] = None, num_shuffles: int = 3, SEED: int = 42,
             output_dir: str = 'cme_files'):
        """
        Load the cme_files from a CSV file, split it, shuffle it, and save the shuffled sets to CSV files.

        :param file_path: Path to the CSV file to load.
        :param num_folds: The number of cme_data_folds to split the cme_files into. If None, the cme_files is not split into cme_data_folds.
        ...
        if num_folds is None:
            ...
            for fold, data in data_splits.items():
                ...
                # Save the shuffled cme_files to CSV files inside the fold directory
                self.save_shuffled_data_to_csv(*shuffled_data, dir_name=fold_dir)

    def load_from_dir(self, dir_path: str):
        """
        Load the shuffled cme_files sets from CSV files in a specified directory.

        :param dir_path: Directory path containing the shuffled CSV files.
        :return: cme_files = (train_x, train_y, val_x, val_y, test_x, test_y)
        """
    ...
    def load_fold_from_dir(self, dir_path: str, fold_id: int):
        ...
        :param fold_id: the fold to load cme_files from
        :param dir_path: Directory path containing the shuffled CSV files.
    ...
    def combine(self, *data: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Parameters:
        - cme_files (np.ndarray): Arbitrarily long list of numpy arrays. x arrays should be at even indices and y arrays at odd indices.
    ...
    def read_data(self, file_path: str, num_folds=None):
        """
        Read cme_files from a CSV file and split it into training, validation, and test sets.
        ...
        if num_folds is None:
            # Split the cme_files into training, validation, and test sets
    ...
    def split_data(self, df):
        """
        Splits the cme_files into training, validation, and test sets according to the specified rules.

        :param:
        - df: DataFrame containing the cme_files. Assumes 'log_peak_intensity' is the target column.

        :return:
        - train_x, train_y, val_x, val_y, test_x, test_y: Numpy arrays containing the split cme_files
        """
    ...
    def split_data_folds(self, df: pd.DataFrame, num_folds: int = 3) -> Dict[str, Any]:
        """
        Splits the cme_files into training, validation, and test sets according to the specified number of cme_data_folds.
        Each fold's test set is picked sequentially rather than at random.

        :param df: DataFrame containing the cme_files. Assumes 'log_peak_intensity' is the target column.
        :param num_folds: The number of cme_data_folds to use for splitting the cme_files.

        :return: A dictionary containing the split cme_files for each fold.
        """
        # Sort the DataFrame by 'log_peak_intensity' in descending order
        df_sorted = df.sort_values(by='log_peak_intensity', ascending=False).reset_index(drop=True)
    ...
    def shuffle_sets(self, train_x, train_y, val_x, val_y, test_x, test_y, num_shuffles=1, SEED=None):
        """
        Shuffle the cme_files within each of the training, validation, and test sets.
    ...
    def save_shuffled_data_to_csv(self, train_x, train_y, val_x, val_y, test_x, test_y, dir_name=''):
        """
        Save the shuffled cme_files sets to CSV files within a specified directory.
    ...
    def read_shuffled_data_from_csv(self, train_file, val_file, test_file):
        """
        Read the shuffled cme_files sets from CSV files.
