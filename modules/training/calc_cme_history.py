...
def parse_datetime(date_str, format_str: str = "%m/%d/%Y %H:%M") -> datetime:
    """
    Parse a date string into a datetime object, or return the input if it is already a datetime.

    Parameters:
    - date_str (str or datetime): The date string or datetime object to parse.
    - format_str (str): The format of the date string.

    Returns:
    - datetime: The parsed datetime object or the original datetime object.
    """
...
def cme_counts_in_timeframe(df: pd.DataFrame, current_time, hours: int) -> int:
    """
    ...
    if not current_dt:
        return 0  # Handle None or invalid dates
...
def cme_counts_with_speed_threshold(df: pd.DataFrame, current_time, hours: int, speed: int) -> int:
    """
...
def max_cme_speed_in_timeframe(df: pd.DataFrame, current_time, hours: int) -> int:
    """
...
def cme_statistics_for_row(df: pd.DataFrame, current_time) -> Tuple[int, int, int, int]:
    """
    ...
    - current_time (str or datetime): The current time to use as a reference.

    Returns:
    - Tuple[int, int, int, int]: A tuple containing the number of CMEs in the past month,
      the number of CMEs in the past 9 hours, the number of CMEs with speed over 1,000 km/s
      in the past 9 hours, and the maximum CME speed in the past day.
    """
...
def calculate_average_duration(df: pd.DataFrame,
    ...
    Args:
        df (pd.DataFrame): DataFrame containing the event cme_files.
