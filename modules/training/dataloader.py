...
class DatasetGenerator:
    ...
    def get_val_data(self, prob: float = .25):
        """
        Get validation cme_files using stratified sampling.
        Sort the labels in descending order, and randomly pick one out of every 1/prob.
        Also sorts the sample weights and carries them to the validation cme_files.
    ...
    def augment_data_by_perturb(self, X: np.ndarray, y: np.ndarray, threshold: float = 1.5, n_augment: int = 500,
        ...
        # Initialize lists to store augmented cme_files
        X_augment = []
        y_augment = []

        # Generate synthetic points
        for _ in range(n_augment):
            # Randomly choose a cme_files point from the high_norm_indices
        ...
        # Combine original and augmented cme_files
    ...
    def augment_data_by_interpol(self, X: np.ndarray, y: np.ndarray, threshold: float = 1.5, n_augment: int = 500) -> \
        ...
        # Generate synthetic points
        for _ in range(n_augment):
            # Randomly choose two cme_files points from the high_norm_indices, weighted by selection_probabilities
