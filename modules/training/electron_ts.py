...
# Define a function to load and preprocess the electron and proton fluxes cme_files
def load_flux_data(filepath: str) -> pd.DataFrame:
    """
    Load the electron and proton fluxes cme_files from a file using spaces as delimiters.

    :param filepath: The path to the cme_files file.
    :return: A pandas DataFrame with the preprocessed cme_files.
    """
...
# Define a function to plot the electron flux cme_files
def plot_electron_flux(df_flux: pd.DataFrame, cme_time: datetime, start_hours_before: int = 2,
                       end_hours_after: int = 3) -> None:
    """
    Plot the electron flux cme_files for a specific time range around a CME event.
...
def plot_fluxes(df_flux: pd.DataFrame, cme_time: datetime, start_hours_before: int, end_hours_after: int,
    ...
    """
    Plot the electron and proton flux cme_files for a specific time range around a CME event in subplots and save the plots.
    ...
    # Iterate over subplots to set titles, date format, and handle no cme_files cases
    for i, ax in enumerate(axs):
        ...
        # Determine whether to plot cme_files or display 'No Data'
...
def add_electron_flux_features_to_SEP(df_flux: pd.DataFrame, sep_df: pd.DataFrame,
    ...
    For each CME event in the SEP DataFrame, this function extracts electron flux cme_files
    for the specified channels for two hours leading up to the CME. The cme_files is sampled
    in 5-minute intervals. This information is then added as new columns to the SEP DataFrame.

    Args:
        df_flux (pd.DataFrame): DataFrame containing electron and proton flux cme_files.
        sep_df (pd.DataFrame): DataFrame containing SEP events cme_files.
    ...
    for index, row in sep_df.iterrows():
        ...
        for channel in channels:
            # Resample the cme_files to 5-minute intervals
