from ts_modeling import (build_full_dataset,
                         create_rnns,
                         create_hybrid_model,
                         evaluate_model,
                         process_sep_events,
...
def main():
    """
    Main function to run the E-rnn model
    :return:
    """
    for inputs_to_use in [['e0.5'], ['e0.5', 'p']]:
        for add_slope in [True, False]:
            for cme_speed_threshold in [0, 500]:
                ...
                # Initialize wandb
                wandb.init(project="rnn-ts-cme", name=experiment_name, config={
                ...
                rnn_tsf_extractor = create_rnns(input_dims=n_features, output_dim=0)
                ...
                X_subtrain_rnn = prepare_hybrid_inputs(X_subtrain,
                                                       ...
                                                       mlp_input_dim=n_cme_features)
                X_val_rnn = prepare_hybrid_inputs(X_val, tsf_extractor_type='rnn',
                                                  tsf_input_dims=n_features,
                                                  with_slope=add_slope,
                                                  mlp_input_dim=n_cme_features)
                X_train_rnn = prepare_hybrid_inputs(X_train, tsf_extractor_type='rnn',
                                                    tsf_input_dims=n_features,
                                                    with_slope=add_slope,
                                                    mlp_input_dim=n_cme_features)
                X_test_rnn = prepare_hybrid_inputs(X_test, tsf_extractor_type='rnn',
                                                   tsf_input_dims=n_features,
                                                   with_slope=add_slope,
                                                   mlp_input_dim=n_cme_features)
                ...
                final_rnn_tsf_extractor = create_rnns(input_dims=n_features, output_dim=0)
